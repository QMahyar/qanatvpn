...
| Build | Flutter + Go + Gomobile | `gomobile` (SagerNet fork), `geodat2srs` | `make lib_android` → `libbox.aar` → `android/app/libs/` → `flutter build apk --split-per-abi` (no legacy aar — API 21 < minSdk 24, dead weight) |
